...
- For daily use, prefer `scripts/start-routine-local.ps1`, `scripts/status-routine-local.ps1`, and `scripts/stop-routine-local.ps1` when managing both systems together.
- The read-only-only commands `scripts/start-readonly-status.ps1`, `scripts/status-readonly-status.ps1`, and `scripts/stop-readonly-status.ps1` remain valid.
...
- If using `start-routine-local.ps1`, confirm phones or LAN devices still open only `http://192.168.50.202:4173/`, while OpenCode remains available only at `http://127.0.0.1:4096/` on this PC.
